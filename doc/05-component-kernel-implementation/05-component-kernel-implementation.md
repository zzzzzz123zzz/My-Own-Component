...
For my kernel implementation of CaseAnalyze, I chose the following representation:

A Map<String, Integer> to store suspects and their suspicion levels
A Set<String> to store clues

I selected this representation because Map makes it easy to add, remove, and
look up suspects efficiently, while Set naturally avoids duplicate clues.
Both structures are OSU components, so they work well with the software design
discipline and make kernel methods straightforward to implement.

Convention:
   suspects != null and clues != null
   All keys in suspects are non-null Strings
   All suspicion values are >= 0

Correspondence:
   suspectCount() = suspects.size()
   clueCount() = clues.size()
   getSuspicion(name) = suspects.value(name)
