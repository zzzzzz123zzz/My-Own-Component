...
For my proof-of-concept, I chose to implement the `CaseAnalyze` component.
I selected this design because it aligns with my personal interests in detective stories such as *Detective Conan*,
and it allows me to model the logic of clues and suspects in a simple but structured way.
Compared to my other ideas, this one is both interesting and feasible to complete within the project timeline.

The CaseAnalyze component manages suspects and clues using OSU components like `Set` and `Map`.
In this proof-of-concept, I focused on demonstrating several core methods such as
`addSuspect`, `addClue`, `analyzeClues`, and `generateReport`.
This shows that my design can realistically be implemented following the OSU discipline.
